...
class FirestoreUserService:
    ...
    async def create_user_avatar(self, user_id: int, avatar_base64: str) -> Optional[str]:
        try:
            ...
            await asyncio.to_thread(doc_ref.set, avatar_data)
            ...
            await asyncio.to_thread(user_doc_ref.set, user_data, merge=True)

            logger.info(
                "Avatar stored in Firestore for user %s, doc_id: %s",
                user_id,
                doc_ref.id,
            )
            return doc_ref.id
            ...
            logger.error(
                "Error storing avatar in Firestore for user %s: %s", user_id, e
            )
            return None
    ...
    async def get_user_avatar(self, user_id: int) -> Optional[str]:
        try:
            user_doc = await asyncio.to_thread(
                self.db.collection(self.users_collection).document(str(user_id)).get
            )
            ...
            avatar_doc = await asyncio.to_thread(
    ...
    async def update_user_avatar(self, user_id: int, avatar_base64: str) -> bool:
        try:
            user_doc_ref = self.db.collection(self.users_collection).document(
                str(user_id)
            )
            ...
            if user_data and user_data.get("current_avatar_id"):
                ...
                await asyncio.to_thread(old_avatar_ref.update, {
                    "is_active": False,
                    "updated_at": datetime.utcnow(),
                })
            ...
            await asyncio.to_thread(new_avatar_ref.set, new_avatar_data)
            ...
            logger.info(
                "Avatar updated for user %s, new doc_id: %s", user_id, new_avatar_ref.id
            )
            return True

        except Exception as e:
            logger.error(
                "Error updating avatar in Firestore for user %s: %s", user_id, e
            )
            return False

    async def delete_user_avatar(self, user_id: int) -> bool:
        try:
            user_doc_ref = self.db.collection(self.users_collection).document(
                str(user_id)
            )
            ...
            if avatar_id:
                avatar_ref = self.db.collection(self.avatars_collection).document(
                    avatar_id
                )
                await asyncio.to_thread(avatar_ref.update, {
                    "is_active": False,
                    "updated_at": datetime.utcnow(),
                })
            ...
            logger.error(
                "Error deleting avatar from Firestore for user %s: %s", user_id, e
            )
            return False