...
class FirestoreUserService:
    ...
    def create_user_avatar(self, user_id: int, avatar_base64: str) -> Optional[str]:
        try:
            ...
            logger.info(
                _(
                    f"Avatar stored in Firestore for user {user_id}, doc_id: {doc_ref.id}"  # noqa: E501
                )
            )
            return doc_ref.id
            ...
            logger.error(
                _(
                    f"Error storing avatar in Firestore for user {user_id}: {e}"  # noqa: E501
                )
            )
            return None
    ...
    def get_user_avatar(self, user_id: int) -> Optional[str]:
        try:
            ...
            if not user_doc.exists:
                logger.warning(
                    _(
                        f"User {user_id} not found in Firestore"  # noqa: E501
                    )
                )
                return None
            ...
            if not avatar_doc.exists:
                logger.warning(
                    _(
                        f"Avatar {avatar_id} not found for user {user_id}"  # noqa: E501
                    )
                )
                return None
            ...
            logger.error(
                _(
                    f"Error getting avatar from Firestore for user {user_id}: {e}"  # noqa: E501
                )
            )
            return None
    ...
    def update_user_avatar(self, user_id: int, avatar_base64: str) -> bool:
        try:
            ...
            logger.info(
                _(
                    f"Avatar updated for user {user_id}, new doc_id: {new_avatar_ref.id}"  # noqa: E501
                )
            )
            return True
            ...
            logger.error(
                _(
                    f"Error updating avatar in Firestore for user {user_id}: {e}"  # noqa: E501
                )
            )
            return False
    ...
    def delete_user_avatar(self, user_id: int) -> bool:
        try:
            ...
            if avatar_id:
                ...
                avatar_ref.update({"is_active": False, "updated_at": datetime.utcnow()})

            user_doc_ref.update(
                {"current_avatar_id": None, "updated_at": datetime.utcnow()}
            )

            logger.info(
                _(
                    f"Avatar deleted for user {user_id}"  # noqa: E501
                )
            )
            return True
            ...
            logger.error(
                _(
                    f"Error deleting avatar from Firestore for user {user_id}: {e}"  # noqa: E501
                )
            )
            return False
