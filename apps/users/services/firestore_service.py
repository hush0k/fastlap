...
class FirestoreUserService:
    ...
    def create_user_avatar(self, user_id: int, avatar_base64: str) -> Optional[str]:
        try:
            ...
            logger.info(
                f"Avatar stored in Firestore for user {user_id}, doc_id: {doc_ref.id}"
            )
            return doc_ref.id
    ...
    def update_user_avatar(self, user_id: int, avatar_base64: str) -> bool:
        try:
            ...
            logger.info(
                f"Avatar updated for user {user_id}, new doc_id: {new_avatar_ref.id}"
            )
            return True
    ...
    def delete_user_avatar(self, user_id: int) -> bool:
        try:
            ...
            logger.error(
                f"Error deleting avatar from Firestore for user {user_id}: {e}"
            )
            return False
