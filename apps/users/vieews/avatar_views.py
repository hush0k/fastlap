        ...
        "multipart/form-data": {
            ...
            "properties": {
                "avatar": {
                    "type": "string",
                    "format": "binary",
                    "description": _("Avatar image file (max 2MB)"),
                }
            },
            "required": ["avatar"],
        }
...
def upload_avatar(request):
    ...
    if not avatar_file:
        return Response(
            {"error": __("No avatar file provided")}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if success:
        ...
        return Response(
            {"message": __("Avatar uploaded successfully"), "avatar": base64_avatar}
        )
    else:
        return Response(
            {"error": __("Failed to upload avatar")},
...
def get_avatar(request):
    ...
    if avatar_base64:
        ...
        return Response(
            {"error": __("Avatar not found")}, status=status.HTTP_404_NOT_FOUND
        )
...
def delete_avatar(request):
    ...
    if success:
        ...
        return Response({"message": __("Avatar deleted successfully")})
    else:
        return Response(
            {"error": __("Failed to delete avatar")},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
