...
class AvatarProcessor:
    ...
    def validate_image(file: InMemoryUploadedFile) -> Tuple[bool, str]:
        ...
        if ext not in settings.ALLOWED_AVATAR_EXTENSIONS:
            return False, f"File extension {ext} not allowed. Allowed: {', '.join(settings.ALLOWED_AVATAR_EXTENSIONS)}"
    ...
    @staticmethod
    def resize_avatar(file: InMemoryUploadedFile, max_size: Tuple[int, int] = (300, 300)) -> BytesIO:
        try:
            file.seek(0)
    ...
    @staticmethod
    def process_avatar(file: InMemoryUploadedFile) -> Tuple[Optional[str], Optional[str]]:
